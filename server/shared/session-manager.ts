...
export class SessionManager {
  ...
  async signIn(
    ...
    try {
      ...
      throw new Error("Invalid credentials or unexpected response from Academia");
    }
    ...
    if (!accessToken || !redirectUrl) {
      throw new Error("Login failed: missing access token from Academia");
    }

    await this.get(`${redirectUrl}&access_token=${accessToken}`);

    if (!this.cookies.has("JSESSIONID")) {
      throw new Error("No JSESSIONID received from Academia");
    }
