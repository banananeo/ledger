...
async function loadCalendarForClient(
  client: PortalClient,
  creds: Credentials,
...
export async function loadAttendanceHtml(
  ...
  return {
    ...
    client,
    metadata,
    didFallback: didFallback.value,
    attendanceTime,
  };
}

export async function loadLoginData(body: Credentials) {
  const creds: Credentials = body;
  const client = new PortalClient(creds.username, creds.password, creds.cookies);
  const didFallback = { value: false };
...
export async function loadCalendarData(body: CalendarRequest) {
...
export async function loadRefreshData(body: Credentials) {
  const creds: Credentials = body;
  const authTime = { value: 0 };
  const { attendanceHtml, client, didFallback: attendanceFallback, attendanceTime } =
    await loadAttendanceHtml(body, authTime);
